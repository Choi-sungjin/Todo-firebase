...
### 이번에 바꾸는 것
- 실제 디자인 변경 적용: `css/style.css` Premium Service UI Overrides (Cinematic) 블록(라인 1475~) — 다크 시네마틱 배경, OTT 스타일 헤더/탭/버튼, 카드 2열 그리드+hover, 모달/입력 프리미엄 톤, 960px 이하 1열 반응형
...
| `index.html` | 모달 입력 | 시작일/마감일/시작 시간/종료 시간 라벨에 icon-calendar, icon-clock SVG 적용. 아이콘 심볼 인라인(헤더 내 hidden SVG)으로 #id 참조 통일 |
| `css/style.css` | picker | --picker-icon-filter 라이트(brightness(0.4) saturate(1.1))/다크(invert(1) brightness(1.2)) 추가, input[type=date/time]::-webkit-calendar-picker-indicator에 적용. 중복 다크 전용 규칙 정리 |
...
### 디자인 토큰 기준값 (권장)
- 타이포: `--fs-display 32px/1.2`, `--fs-title 24px/1.3`, `--fs-body 16px/1.5`, `--fs-caption 13px/1.4`
- 간격: `--space-1 4px`, `--space-2 8px`, `--space-3 12px`, `--space-4 16px`, `--space-6 24px`, `--space-8 32px`, `--space-12 48px`
- radius: `--radius-sm 8px`, `--radius-md 12px`, `--radius-lg 16px`
- 그림자: `--shadow-card 0 8px 24px rgba(0,0,0,.22)`, `--shadow-hover 0 16px 36px rgba(0,0,0,.32)`
- 전환: `--dur-fast 140ms`, `--dur-base 220ms`, `--dur-slow 300ms`, `--ease-standard cubic-bezier(.2,.8,.2,1)`
...
2. UI 변경만 되돌릴 때(선택 롤백): `git restore --source <commit> -- index.html css/style.css css/motion.css js/app.js js/schedule.js`
3. 구조 변경(이동/리네임)까지 되돌릴 때(전체 추적 파일 롤백): `git restore --source <commit> -- .`
4. 구조 변경 중 생긴 신규 파일 정리 필요 시(주의): `git clean -fd` 실행 전 `git clean -fdn`으로 미리보기
5. 로드 순서/핵심 기능 재검증: 탭, CRUD, 타임피커, 스케줄 렌더
6. `HANDOFF.md`에 롤백 사유/범위/결과 기록 (어느 단계까지 롤백했는지 명시)

### Cursor 전달용 롤백 요청 문장
- "아래 커밋 기준으로 UI 변경 파일만 선택 롤백해줘. 데이터 스키마와 Firebase 연결은 유지해."
- "롤백 후 깨진 참조 경로와 script 로드 순서까지 함께 검증해줘."
- "구조 변경까지 롤백이 필요하면 tracked 파일 전체 복구 후, untracked 파일은 목록 확인 뒤 정리해줘."

---

*마지막 업데이트: 2026-03-01, GPT+Cursor 턴 – 모달 날짜/시간 라벨 아이콘, picker 아이콘 라이트/다크 필터 통일*
